	...
	_drawLayer(context, z) {
		...
		if(layer) {
			if (this.perspectiveMode == TiledScene.PERSPECTIVE_ANGLE) {
				...
				var mobs = this.getEntitiesAtLayer(z)
				for(var m in mobs) {
...
// In PERSPECTIVE_OVERHEAD mode, a TiledScene will simply draw all tile levels, subscenes and mobs in order from their z-coordinate
TiledScene.PERSPECTIVE_OVERHEAD = 1
// In PERSPECTIVE_ANGLE mode, the TiledScene will draw all tile levels, subscenes and mobs in order from their z and y coordinates modified their hotspot y coordinate. 
